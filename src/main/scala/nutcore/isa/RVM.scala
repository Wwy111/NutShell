...
object RV32ComInstr extends HasInstrType with HasNutCoreParameter {
  ...
  def COMFORM  = BitPat("b0000000_?????_?????_001_?????_0001011")
  def COMCONJ  = BitPat("b000000000000_?????_010_?????_0001011")

  def COMCMULA = BitPat("b0000000_?????_?????_100_?????_0001011")
  def COMMULS  = BitPat("b0000000_?????_?????_101_?????_0001011")
  def COMMULA  = BitPat("b0000000_?????_?????_110_?????_0001011")
  def COMCMULS = BitPat("b0000000_?????_?????_111_?????_0001011")

  def FCOMCMULA = BitPat("b1000000_?????_?????_100_?????_0001011")
  def FCOMMULS  = BitPat("b1000000_?????_?????_101_?????_0001011")
  def FCOMMULA  = BitPat("b1000000_?????_?????_110_?????_0001011")
  def FCOMCMULS = BitPat("b1000000_?????_?????_111_?????_0001011")


  val comTable = Array(
    COMADD         -> List(InstrR, FuType.comu, COMUOpType.comadd),
    COMSUB         -> List(InstrR, FuType.comu, COMUOpType.comsub),
    COMFORM        -> List(InstrR, FuType.comu, COMUOpType.comform),
